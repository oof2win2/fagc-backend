...
import { CommunityCreatedMessageExtraOpts, CommunityRemovedMessageExtraOpts, ReportMessageExtraOpts, RevocationMessageExtraOpts } from "fagc-api-types"
...
export async function reportCreatedMessage(report: DocumentType<ReportClass, BeAnObject>, opts: ReportMessageExtraOpts): Promise<void> {
	...
	const reportEmbed = new MessageEmbed()
		...
		.addFields(
			...
			{ name: "Admin", value: `<@${opts.admin.id}> | ${opts.admin.username}#${opts.admin.discriminator}`, inline: true },
...
export async function reportRevokedMessage(revocation: DocumentType<RevocationClass, BeAnObject>, opts: RevocationMessageExtraOpts): Promise<void> {
	...
	const revocationEmbed = new MessageEmbed()
		...
		.addFields([
			...
			{ name: "Admin", value: `<@${opts.admin.id}> | ${opts.admin.username}#${opts.admin.discriminator}`, inline: true },
			{ name: "Community", value: `${opts.community.name} (\`${opts.community.id}\`)`, inline: true },
			{ name: "Revoked by", value: `<@${opts.revokedBy.id}> | ${opts.revokedBy.username}#${opts.revokedBy.discriminator}`, inline: true },
		])
		.setTimestamp()
...
export async function communityCreatedMessage(community: DocumentType<CommunityClass, BeAnObject>, opts: CommunityCreatedMessageExtraOpts): Promise<void> {
	...
	const embed = new MessageEmbed()
		...
		.addFields(
			...
			{ name: "Contact", value: `<@${opts.contact.id}> | ${opts.contact.username}#${opts.contact.discriminator}`, inline: true },
		)
...
export async function communityRemovedMessage(community: DocumentType<CommunityClass, BeAnObject>, opts: CommunityRemovedMessageExtraOpts): Promise<void> {
