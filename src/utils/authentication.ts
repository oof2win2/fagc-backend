...
export const MasterAuthenticate = <T extends RouteGenericInterface = RouteGenericInterface>(
	_target: unknown,
	_propertyKey: unknown,
	descriptor: TypedPropertyDescriptor<(
		req: FastifyRequest<T>,
	...
	descriptor.value = async (...args) => {
		...
		if (auth.startsWith("Token ")) {
			const token = auth.slice("Token ".length)
			const authData = await AuthModel.findOne({ api_key: token, api_key_type: "master" })
			const community = await CommunityModel.findOne({ _id: authData?.communityId })
			if (!community) return res.status(401).send({
				statusCode: 401,
				error: "Unauthorized",
