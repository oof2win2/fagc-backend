...
export default class ProfileController {
	@GET({url: "/logs/:count/:since", options: {
		schema: {
	...
	async getLogs(req: FastifyRequest<{
		Params: {
			count: number,
			since: number
		}
	...
	async addWebhook(req: FastifyRequest<{
		Body: {
			id: string
			token: string
		}
		...
		try {
			...
			return res.status(404).send({errorCode: 404, error: "Not Found", message: "Provided webhook could not be found"})
		}
		...
		if (webhooksInSameGuild.length) {
			...
			webhook.send(msg)
			return res.status(403).send({errorCode: 403, error: "Forbidden", message: msg})
		}
	...
	async removeWebhook(req: FastifyRequest<{
		Body: {
			id: string
			token: string
		}
